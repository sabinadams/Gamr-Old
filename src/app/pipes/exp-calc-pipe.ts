...
transform( exp: number = 125, type: string = 'curr' ): any {
 switch ( type ) {
  ...
  case 'getPerc':
   const	past = this.getNextXP( this.getLevel( exp ) - 1 );
   const	perc = Math.trunc( 100 * ( ( exp - past ) / ( this.getNextXP( this.getLevel( exp ) ) - past ) ) );
   return perc === 100 ? '0' : perc.toString();
  }
 }

  getNextXP( lvl: number ): number { return Math.trunc( Math.pow( lvl + 1, 3 ) + Math.pow( lvl / 2, 3) ); }

  getLevel( xp: number ): number { return Math.trunc( Math.pow( xp, 1 / 3 ) ); }
